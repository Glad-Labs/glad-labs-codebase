...
const PostList = ({ posts }) => {
  ...
  if (!posts || posts.length === 0) {
    return <p className="text-center text-gray-400">No posts found. Check back soon!</p>;
  }

  return (
    <ul className="space-y-8 max-w-4xl mx-auto">
      {posts.map((post) => (
        <li key={post.Slug} className="border-b border-gray-700 pb-4 last:border-b-0">
          ...
          <Link href={`/posts/${post.Slug}`} className="text-cyan-500 hover:text-cyan-400 mt-4 inline-block">
            Read more &rarr;
          </Link>
        </li>
      ))}
