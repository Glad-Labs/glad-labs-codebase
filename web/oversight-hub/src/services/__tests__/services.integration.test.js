...
describe('settingsService Integration Tests', () => {
  beforeAll(() => {
    if (!TEST_AUTH_TOKEN) {
      console.warn('⚠️  REACT_APP_TEST_AUTH_TOKEN not set. Some tests will be skipped.');
    }
    if (!API_URL.includes('localhost') && !API_URL.includes('127.0.0.1')) {
      console.warn('⚠️  Tests configured for remote API. Make sure backend is running.');
    }
  });
  ...
  describe('Settings CRUD Operations', () => {
    test.skipIf(!TEST_AUTH_TOKEN)('should fetch all settings from backend', async () => {
    ...
    test.skipIf(!TEST_AUTH_TOKEN)('should fetch specific setting by key', async () => {
      setAuthToken(TEST_AUTH_TOKEN);
    ...
    test.skipIf(!TEST_AUTH_TOKEN)('should update existing setting', async () => {
      setAuthToken(TEST_AUTH_TOKEN);
    ...
    test.skipIf(!TEST_AUTH_TOKEN)('should handle missing settings gracefully', async () => {
      setAuthToken(TEST_AUTH_TOKEN);
      
      const setting = await getSettingWithDefault('nonexistent_key', 'default_value');
  ...
  describe('Bulk Settings Operations', () => {
    test.skipIf(!TEST_AUTH_TOKEN)('should update multiple settings at once', async () => {
      setAuthToken(TEST_AUTH_TOKEN);
      const updates = {
  ...
  describe('Error Logging', () => {
    ...
    test.skipIf(!TEST_AUTH_TOKEN)('should retrieve error logs from backend', async () => {
      setAuthToken(TEST_AUTH_TOKEN);
    ...
    test.skipIf(!TEST_AUTH_TOKEN)('should delete specific error log', async () => {
      setAuthToken(TEST_AUTH_TOKEN);
  ...
  describe('Error Handling', () => {
    ...
    test('should handle network errors', async () => {
      ...
      process.env.REACT_APP_API_URL = 'http://invalid-url-xyz-12345.example.com:9999';
  ...
  describe('Response Validation', () => {
    test.skipIf(!TEST_AUTH_TOKEN)('should validate settings response schema', async () => {
      setAuthToken(TEST_AUTH_TOKEN);
    ...
    test.skipIf(!TEST_AUTH_TOKEN)('should validate error log schema', async () => {
      setAuthToken(TEST_AUTH_TOKEN);
...
describe('Performance Tests', () => {
  ...
  test.skipIf(!TEST_AUTH_TOKEN)('settings fetch should complete within 5 seconds', async () => {
    setAuthToken(TEST_AUTH_TOKEN);
