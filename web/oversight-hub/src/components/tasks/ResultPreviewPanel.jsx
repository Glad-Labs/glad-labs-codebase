...
const ResultPreviewPanel = ({
  ...
  const approvalTextareaRef = useRef(null);
  ...
  const ModalWrapper = ({ children }) => (
    <div className="fixed inset-0 z-50 flex items-center justify-center pointer-events-none">
      ...
      />
      {/* Modal */}
      <div className="relative pointer-events-auto bg-gray-900 border-2 border-cyan-500/50 rounded-lg shadow-2xl max-w-4xl w-full mx-4 max-h-[90vh] overflow-hidden flex flex-col">
        {children}
        {/* Close button */}
        <button
          onClick={onClose}
          className="absolute top-4 right-4 text-gray-400 hover:text-gray-200 transition z-10 pointer-events-auto"
          title="Close"
        >
          ✕
        </button>
  ...
  // Task completed - show results in modal
  return (
    <ModalWrapper>
      <div className="h-full flex flex-col bg-gray-900 overflow-hidden">
        {/* Header */}
        ...
        >
          ...
          {/* Approval Section - Inline Form Below Task Preview */}
          {(task.status === 'awaiting_approval' ||
            task.status === 'rejected') && (
            <div className="border-t border-cyan-500/30 pt-6">
              <div className="bg-gradient-to-r from-cyan-900/20 to-blue-900/20 border border-cyan-500/30 rounded-lg p-6 space-y-6">
                <div>
                  ...
                  <p className="text-sm text-gray-300">
                    {task.status === 'rejected'
                      ? 'This task was previously rejected. Review the content and approve to publish, or provide additional feedback.'
                      : 'This task is awaiting your review and approval. Fill out the form below to approve or reject.'}
                  </p>
                </div>
                ...
                <div>
                  ...
                  <textarea
                    ...
                    onChange={(e) => {
                      setApprovalFeedback(e.target.value);
                      // Refocus the textarea after state update to prevent focus loss
                      setTimeout(() => {
                        if (approvalTextareaRef.current) {
                          approvalTextareaRef.current.focus();
                          // Move cursor to the end
                          approvalTextareaRef.current.selectionStart =
                            approvalTextareaRef.current.value.length;
                          approvalTextareaRef.current.selectionEnd =
                            approvalTextareaRef.current.value.length;
                        }
                      }, 0);
                    }}
                    placeholder="e.g., Content looks good, well-written, ready for publishing. Or provide specific feedback for revision."
                    disabled={approvalLoading}
                    rows={4}
        ...
        <div className="border-t border-cyan-500/30 bg-gray-800 p-4 flex gap-3 justify-end">
          ...
          {!['awaiting_approval', 'rejected'].includes(task.status) ? (
            ...
            /* Approval form is shown above in the approval section */
            <p className="text-xs text-gray-400">Use the approval form above</p>
          )}
        </div>
      </div>
    </ModalWrapper>
